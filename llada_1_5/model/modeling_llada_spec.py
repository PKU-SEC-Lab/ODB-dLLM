...
class RotaryEmbeddingM(nn.Module):
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, position_ids: torch.Tensor, current_block_start: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.autocast(q.device.type, enabled=False):
            query_len, key_len = q_.shape[-2], k_.shape[-2]  # could be different if layer_past not None
            pos_sin, pos_cos = self.get_rotary_embedding(key_len-96, q_.device)
            pos_sin = pos_sin.type_as(q_)
            pos_cos = pos_cos.type_as(q_)
            q_position_ids = position_ids[current_block_start:current_block_start + query_len]
            # q_position_ids = position_ids[current_block_start:current_block_start + 32]
            
            q_ = self.apply_rotary_pos_emb(
                # pos_sin[:, :, key_len - query_len : key_len, :],
                # pos_cos[:, :, key_len - query_len : key_len, :],
                pos_sin[:, :, q_position_ids, :],
                pos_cos[:, :, q_position_ids, :],
                q_,
            )
...
class LLaDABlock(nn.Module):
    ...
    def attention(
        ...
        if layer_past is None:
            ...
            present = (k_initial, v_initial) if use_cache else None #present: None
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def forward(
        ...
        if position_ids is None:
            if self._activation_checkpoint_fn is not None:
                att, cache = self._activation_checkpoint_fn(  # type: ignore
                    self.attention, q, k, v, attention_bias, current_block_start=current_block_start, 
                    layer_past=layer_past, use_cache=use_cache,replace_position=replace_position
                )
            else:
                att, cache = self.attention(q, k, v, attention_bias, current_block_start=current_block_start, 
                                            layer_past=layer_past, use_cache=use_cache,replace_position=replace_position)
                
        else:
            if self._activation_checkpoint_fn is not None:
                att, cache = self._activation_checkpoint_fn(  # type: ignore
                    self.attention, q, k, v, attention_mask=attention_mask, position_ids=position_ids, current_block_start=current_block_start,
                    layer_past=layer_past, use_cache=use_cache,replace_position=replace_position
                )
            else:
                att, cache = self.attention(q, k, v, attention_mask=attention_mask, position_ids=position_ids, current_block_start=current_block_start, 
                                            layer_past=layer_past, use_cache=use_cache,replace_position=replace_position)
