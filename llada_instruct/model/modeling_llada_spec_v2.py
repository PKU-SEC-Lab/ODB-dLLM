...
class RotaryEmbeddingM(nn.Module):
    """
    [Rotary positional embeddings (RoPE)](https://arxiv.org/abs/2104.09864).
    """
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, position_ids: torch.Tensor, current_block_start: int) -> Tuple[torch.Tensor, torch.Tensor]:
        if self.config.rope_full_precision:
            q_, k_ = q.float(), k.float()
        else:
            q_, k_ = q, k
        ...
        with torch.autocast(q.device.type, enabled=False):
            pos_sin, pos_cos = self.get_rotary_embedding(current_block_start + 32, q_.device)
            pos_sin = pos_sin.type_as(q_)
            pos_cos = pos_cos.type_as(q_)
...
class LLaDABlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        if self.config.rope:
            ...
            self.rotary_emb_2 = RotaryEmbeddingM(config, self.__cache)
    ...
    def attention(
        ...
        if position_ids is None:
            ...
            if layer_past is None:
                additional_k = k[:, :, current_block_start:current_block_start+32].repeat(1,1,3,1) 
                k_initial = torch.cat([k[:, :, :current_block_start], additional_k, k[:, :, current_block_start:]], dim=2)
                additional_v = v[:, :, current_block_start:current_block_start+32].repeat(1,1,3,1) 
                v_initial = torch.cat([v[:, :, :current_block_start], additional_v, v[:, :, current_block_start:]], dim=2)
                present = (k_initial, v_initial) if use_cache else None
            ...
            if self.config.rope:
                q, k = self.rotary_emb_2(q, k, position_ids, current_block_start)
            ...
            if layer_past is not None: 
                ...
                if replace_position is None:
                    ...
                    replace_indices = torch.arange(current_block_start, current_block_start + 128, device=k.device)
                    if decode_length == 129 or decode_length == 127:
                        ...
                        temp_k = torch.cat([past_key[:, :, :current_block_start], k, past_key[:, :, current_block_start+128:]], dim=2)
                        k = temp_k
                        temp_v = torch.cat([past_value[:, :, :current_block_start], v, past_value[:, :, current_block_start+128:]], dim=2)
                        v = temp_v
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def forward(
        ...
        if position_ids is None:
            if self._activation_checkpoint_fn is not None:
                att, cache = self._activation_checkpoint_fn(  # type: ignore
                    self.attention, q, k, v, attention_bias, current_block_start=current_block_start, decode_length=decode_length,
                    layer_past=layer_past, use_cache=use_cache,replace_position=replace_position
                )
            else:
                att, cache = self.attention(q, k, v, attention_bias, current_block_start=current_block_start,  decode_length=decode_length,
                                            layer_past=layer_past, use_cache=use_cache,replace_position=replace_position)
        else:
            if self._activation_checkpoint_fn is not None:
                att, cache = self._activation_checkpoint_fn(  # type: ignore
                    self.attention, q, k, v, attention_mask=attention_mask, position_ids=position_ids, current_block_start=current_block_start, decode_length=decode_length,
                    layer_past=layer_past, use_cache=use_cache,replace_position=replace_position
                )
            else:
                att, cache = self.attention(q, k, v, attention_mask=attention_mask, position_ids=position_ids, current_block_start=current_block_start, decode_length=decode_length, 
                                            layer_past=layer_past, use_cache=use_cache,replace_position=replace_position)
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                if (
                    ...
                    # shape: (batch_size, seq_len, d_model)
                    if position_ids is None:
                        x, cache = block(x, attention_bias=attention_bias, current_block_start=current_block_start, 
                                         layer_past=layer_past, use_cache=use_cache,replace_position=replace_position)
                    else:
                        x, cache = block(x, attention_mask=attention_mask, layer_past=layer_past, 
